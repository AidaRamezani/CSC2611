...
from changepoint.changepoint.utils.ts_stats import ts_stats_significance, ts_stats_significance_bootstrap
...
class MeanShiftModel(object):
    ...
    def compute_balance_mean(self, ts, t):
        ...
        return abs(np.mean(ts[t + 1:]) - np.mean(ts[:t + 1]))
...
class SuccessiveMeanShiftModel(MeanShiftModel):
    def compute_balance_mean(self, ts, t):
        """ Compute the balance. The right end - current point."""
        """ For changed words we expect an increase in the mean, and so only 1 """
        return np.mean(ts[t + 1:]) - ts[t]